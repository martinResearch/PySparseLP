...
def generate_random_lps(nb_problems=10, get_dual_sol=True):
    ...
    for id in range(nb_problems):
        print(f'generating problem {id}')
        ...
        if get_dual_sol:
            ...
            assert(abs(lp.costsvector.dot(scipy_sol) +
                       lp_dual.costsvector.dot(y_ineq)) < 1e-8)
...
class DeepPrimalDual(nn.Module):
    def __init__(self, nch_primal=(4, 4), nch_dual=(4, 4), has_bounds=False, coef_increase_primal=1, coef_increase_dual=1):
    ...
    def forward(self, lp, nb_max_iter, loss_decay=0.90, beta=1, cheat=False, std_y=1.0):
        ...
        if self.has_bounds:
            ...
            x = beta * \
                (torch.rand(a.shape[1], 1, dtype=torch.float32) -
                 0.5) * (ub - lb) + 0.5 * (ub + lb)
        else:
        ...
        if cheat:
            ...
            if lp.dual_solution is not None:
                y = alpha * \
                    (torch.FloatTensor(lp.dual_solution[:, None])) + (1 - alpha) * y

        # x = torch.zeros(a.shape[1], 1, dtype=torch.float32)

        y_h = torch .zeros(self.nch_dual[-1], a.shape[0],
                           dtype=torch.float32)
        ...
        for n_iter in range(nb_max_iter):
            ...
            for i in range(len(self.primal_convs)):
                ...
                x_h = F.relu(self.primal_convs[i](x_h[
                             None, :, :])).squeeze(dim=0)
        ...
        loss_primal = mean_loss_primal + \
            self.coef_increase_primal * torch.mean(increases_primal)
        ...
        if lp.dual_solution is not None:
            ...
            loss_dual = mean_loss_dual + \
                self.coef_increase_dual * torch.mean(increases_dual)
...
def train(net, max_iter=1000,
    ...
    for n_epoch in range(max_iter):
        ...
        for id_batch in range(nb_batch):
            batch_lps = all_train_lps[nb_lp_per_batch *
                                      id_batch:nb_lp_per_batch * (id_batch + 1)]
            optimizer.zero_grad()
            losses = []
            ...
            for lp in batch_lps:
                x, y, mean_loss, distances_primal, distances_dual = net(
                    lp, nb_max_iter=nb_max_iter_per_lp_train, cheat=cheat_train, beta=0, std_y=0)
            ...
            if id_batch == 0 :
                ...
                fig_train_batch_visu.savefig(os.path.join(outputs_folder,
        ...
        with torch.no_grad():
            ...
            fig_test_batch_visu.savefig(os.path.join(outputs_folder,
                                                     f'test_primal/epoch{n_epoch}_convergence_curves.png'))
            if use_dual_solution:
                ax_test_batch_visu.clear()
...
if __name__ == "__main__":
    np.random.seed(0)
    outputs_folder = os.path.join(os.path.dirname(
        __file__), 'deep_primal_dual_training', datetime.datetime.now().strftime('%Y-%m-%d %H_%M_%S'))
